 ...
 */
class QuickSplicingRecyclerViewAdapter(private val mContext: Context, val mRecyclerView: RecyclerView) :
    RecyclerView.Adapter<QuickSplicingRecyclerViewAdapter.ViewHolder>(), IItemTouchHelperAdapterCallback {
    ...
    override fun onSwiped(postion:Int) {
        mClipInfos?.removeAt(postion) //移除数据
        postion.let {
            mOnItemClickListener?.onItemDelete(it)
            notifyItemRemoved(it)//刷新数据移除
        }
