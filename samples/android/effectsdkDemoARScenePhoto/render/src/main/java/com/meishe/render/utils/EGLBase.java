package com.meishe.render.utils;
...
public class EGLBase {    // API >= 17
    ...
    public static class EglSurface {
        ...
        public void presentTime(long presentTime) {
            mEgl.presentTime(mEglSurface, presentTime);
        }
    ...
    private void presentTime(final EGLSurface surface, long presentTime) {
        if (!EGLExt.eglPresentationTimeANDROID(mEglDisplay, surface, presentTime))
            Log.e(TAG, "eglPresentationTimeANDROID() failed, errno=0x" + EGL14.eglGetError());
    }
    ...
    private EGLConfig getConfig(final boolean with_depth_buffer, final boolean isRecordable) {
        ...
        if (isRecordable && (Build.VERSION.SDK_INT >= 18)) {// MediaCodec入力用Surfaceの場合
