    ...
    public List<MediaData> getMediaDataList(int position) {
        ...
        if (fragment != null) {
            ...
            if (adapter != null) {
                ...
                Iterator<MediaData> iterable = mediaDataList.iterator();
                while (iterable.hasNext()) {
                    MediaData mediaData = iterable.next();
                    if (!FileUtils.isFileExists(mediaData.getPath())) {
                        ToastUtil.showToast(SelectMediaActivity.this, FileUtils.getFileName(mediaData.getDisplayName()) + getString(R.string.selete_delete));
                        iterable.remove();
                    }
                }
