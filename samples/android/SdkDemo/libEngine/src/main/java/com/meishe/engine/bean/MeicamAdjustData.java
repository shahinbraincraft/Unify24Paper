...
public class MeicamAdjustData implements Cloneable, Serializable, TimelineDataToLocalAdapter<LMeicamAdjustData> {
    public final static String[] JSON_KEY = new String[]{"brightness", "contrast", "saturation", "highlight", "shadow", "blackPoint", "amount", "temperature", "tint"};
    ///调整校色
    ...
    private float mHighlight;  //高光
    @SerializedName("Shadow")
    private float mShadow;  //阴影
    @SerializedName("Blackpoint")
    private float mBlackPoint;  //褐色 -1，0
    @SerializedName("Degree")
    private float mDegree;  //暗角 取值范围0-1
    @SerializedName("Amount")
    private float mAmount;  //锐度 取值范围0-1
    @SerializedName("Temperature")
    private float mTemperature;   //色温
    @SerializedName("Tint")
    private float mTint;    //色调
