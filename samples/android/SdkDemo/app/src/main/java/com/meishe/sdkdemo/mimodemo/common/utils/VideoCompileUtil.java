...
public class VideoCompileUtil {
    public static String getCompileVideoPath() {
        String compilePath = MimoPathUtils.getVideoCompileDirPath();
        if (compilePath == null)
            return null;
    ...
    public static void compileVideo(NvsStreamingContext context,
        ...
        Hashtable<String, Object> compileConfigurations = com.meishe.sdkdemo.utils.VideoCompileUtil.getCompileConfigurations(config);
        context.setCompileConfigurations(compileConfigurations);
        int encoderFlag = 0;
