...
import com.meishe.sdkdemo.themeshoot.utlils.ThemeTimelineUtils;
 ...
 * @Author : zcy
 * @CreateDate : 2020/8/5.
 * @Description :主题拍摄-视频编辑预览。ThemePreview-VideoEditPreview
...
public class ThemePreviewActivity extends BaseActivity {
    ...
    private void initFilterList() {
        ...
        assetFilterAdapter.setOnItemClickListener(new AssetFilterAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                playTimeline(mTimeline, startDuration, mTimelineDuration);
            }
        });
    }
    ...
    private void showSubView(ThemePreviewBean previewBean) {
        ...
        mTimelineDuration = previewBean.getStartDuration() + previewBean.getDuration();
        playTimeline(mTimeline, startDuration, mTimelineDuration);
        ...
        subTittle.setText(previewBean.getName() + "-"+getString(R.string.edit));
...
//        }
        ...
        for (int i = 0; i < shotInfos.size(); i++) {
            ...
            // If there is a variable speed when adding clips, select the duration and add them in a loop
...
//                nvsVideoClip.setExtraVideoRotation(videoRotation);
            ...
            previewBean.setName(getString(R.string.activity_theme_priview_clip) + (i + 1));
        ...
        headBean.setName(getString(R.string.activity_theme_priview_clip_head));
        //片头封面
        ...
        mTimelineDuration = mTimeline.getDuration();
        long endFilterDuration = NumberUtils.parseString2Long(mThemeModel.getEndingFilterLen()) * 1000;
        if (!TextUtils.isEmpty(endingFilter)) {
            NvsTimelineVideoFx nvsTimelineVideoFx = mTimeline.addPackagedTimelineVideoFx(mTimelineDuration-endFilterDuration,
                    mTimelineDuration, endingFilter);
        ...
        Log.d(TAG,"endDuration==="+ mTimelineDuration +"   startDuration=="+startDuration+" endDuration="+ mTimelineDuration);
        playTimeline(mTimeline, startDuration, mTimelineDuration);
    ...
    protected void onResume() {
        ...
        if (mStreamingContext != null && mTimeline != null && playView != null) {
            if (playView.isPlaying()) {
                playTimeline(mTimeline, mStreamingContext.getTimelineCurrentPosition(mTimeline), mTimelineDuration);
            }
        }
    }
    ...
    protected void initListener() {
        ...
        themeCompile.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String fileName ="themeShoot"+System.currentTimeMillis()+".mp4";
                mCompileVideoPath = com.meishe.engine.util.PathUtils.getVideoSavePath(fileName);
                VideoCompileUtil.compileVideo(mStreamingContext, mTimeline, mCompileVideoPath, 0 , mTimeline.getDuration(),
                        NvsStreamingContext.COMPILE_VIDEO_RESOLUTION_GRADE_1080, -1);
            }
        });
