            ...
            mVideoFragment.setAssetEditListener(new VideoFragment.AssetEditListener() {
                ...
                public void onAssetSelected(PointF curPoint) {
                    ...
                    if (!mIsInnerDrawRect) {
                        ...
                        if (curCaption != null && curCaption instanceof NvsTimelineCaption) {
                            mCurCaption = (NvsTimelineCaption) curCaption;
                        } else {
                            mCurCaption = null;
                        }
...
//                    mIsInnerDrawRect = false;
    ...
    /**
     * 根据字幕信息生成一个关键帧对象
     * Generate a KeyFrameInfo object based on NvsTimelineCaption information
     *
     * @param caption
     * @return
     */
    private KeyFrameInfo generateKeyFrameInfo(NvsTimelineCaption caption) {
        return new KeyFrameInfo().setScaleX(caption.getScaleX())
                ...
                .setTranslation(caption.getCaptionTranslation()).setOpacity(caption.getOpacity());
    }
...
//                }).show();
                ...
                BackupData.instance().setCurrentTime(mStreamingContext.getTimelineCurrentPosition(mTimeline));
                bundle = new Bundle();
                bundle.putBoolean("tradition_caption", true);
...
//                }).show();
                ...
                BackupData.instance().setCurrentTime(mStreamingContext.getTimelineCurrentPosition(mTimeline));
                bundle = new Bundle();
                bundle.putBoolean("tradition_caption", false);
                bundle.putBoolean("isAdd",true);
                AppManager.getInstance().jumpActivityForResult(AppManager.getInstance().currentActivity(), CaptionStyleActivity.class, bundle, REQUESTCAPTIONSTYLE);
                break;
            ...
            case R.id.iv_confirm_curve:
                ...
                changeRectVisible();
                break;
            case R.id.iv_confirm_curve_adjust:
    ...
    private void addFrame() {
        ...
        addKeyFrameOnCurrentTime(duration, mCurCaption.getCaptionTranslation(), mCurCaption.getScaleX(),
                mCurCaption.getScaleY(), mCurCaption.getRotationZ(), mCurCaption.getOpacity());
        long timelineCurrentPosition = mStreamingContext.getTimelineCurrentPosition(mTimeline);
        if (captionInfo != null) {
            KeyFrameInfo keyFrameInfo = new KeyFrameInfo()
                    ...
                    .setOpacity(mCurCaption.getOpacity());

            captionInfo.putKeyFrameInfo(timelineCurrentPosition, keyFrameInfo);
            disableKeyFrameBezierData(captionInfo.getKeyFrameInfo(), timelineCurrentPosition, true);
        }
        int alignVal = mCurCaption.getTextAlignment();
        mVideoFragment.setAlignIndex(alignVal);
        updateCaptionBoundingRect();
    ...
    private void deleteFrame() {
        ...
        if (captionInfo != null) {
            ...
            disableKeyFrameBezierData(keyFrameMap, timelineCurrentPosition, false);
        }
        CaptionTimeSpanInfo spanInfo = getCurrentTimeSpanInfo();
        if (captionInfo != null && spanInfo != null && spanInfo.mTimeSpan != null) {
            spanInfo.mTimeSpan.setKeyFrameInfo(captionInfo.getKeyFrameInfo());
    ...
    private void disableKeyFrameBezierData(Map<Long, KeyFrameInfo> keyFrameMap, long currentTimelinePosition
        ...
        for (Map.Entry<Long, KeyFrameInfo> entry : keyFrameMap.entrySet()) {
            ...
            } else if (key < currentTimelinePosition) {
                if (lastKeyFrame == -1 || lastKeyFrame > key) {
                    lastKeyFrame = key;
                }
            } else {
                if (currentKeyFrame == -1 && addFlag) {
                    currentKeyFrame = key;
                }
            }
        }
        if (nextKeyFrame > 0 && !addFlag) {
            keyFrameMap.get(nextKeyFrame).setBackwardControlPointX(-1);
        }
        if (lastKeyFrame > 0 && !addFlag) {
            keyFrameMap.get(lastKeyFrame).setForwardControlPointX(-1);
        }
        if (currentKeyFrame > 0 && addFlag) {
            refreshNewAddKeyFrameControlPoint(keyFrameMap.get(currentKeyFrame), currentKeyFrame);
        }
    }

    /**
     * 新增的关键帧检索是否存在控制点信息，存在则应用到应用层
     *
     * @param keyFrameInfo
     * @param currentKeyFrame
     */
    private void refreshNewAddKeyFrameControlPoint(KeyFrameInfo keyFrameInfo, long currentKeyFrame) {
        if (keyFrameInfo == null) return;
        refreshSingleControlPoint(keyFrameInfo, TRANS_X, currentKeyFrame);
        refreshSingleControlPoint(keyFrameInfo, TRANS_Y, currentKeyFrame);
//        refreshSingleControlPoint(keyFrameInfo, SCALE_X, currentKeyFrame);
//        refreshSingleControlPoint(keyFrameInfo, SCALE_Y, currentKeyFrame);
//        refreshSingleControlPoint(keyFrameInfo, ROTATION_Z, currentKeyFrame);
    }

    private void refreshSingleControlPoint(KeyFrameInfo keyFrameInfo, String controlType, long currentKeyFrame) {
        if (keyFrameInfo == null || mCurCaption == null) return;
        Log.d(TAG, "refreshSingleControlPoint type:" + controlType);
        long inPoint = mCurCaption.getInPoint();
        NvsControlPointPair controlPointPair = mCurCaption.getKeyFrameControlPoint(controlType, currentKeyFrame);
        if (controlPointPair != null) {
            //1.前置
            keyFrameInfo.setForwardControlPointX(controlPointPair.forwardControlPoint.x + inPoint);
            if (TextUtils.equals(controlType, TRANS_X)) {
                keyFrameInfo.setForwardControlPointYForTransX(controlPointPair.forwardControlPoint.y);
            } else if (TextUtils.equals(controlType, TRANS_Y)) {
                keyFrameInfo.setForwardControlPointYForTransY(controlPointPair.forwardControlPoint.y);
            }
            //2.后置
            keyFrameInfo.setBackwardControlPointX(controlPointPair.backwardControlPoint.x + inPoint);
            if (TextUtils.equals(controlType, TRANS_X)) {
                keyFrameInfo.setBackwardControlPointYForTransX(controlPointPair.backwardControlPoint.y);
            } else if (TextUtils.equals(controlType, TRANS_Y)) {
                keyFrameInfo.setBackwardControlPointYForTransY(controlPointPair.backwardControlPoint.y);
            }
            Log.d(TAG, "refreshSingleControlPoint === fP:" + keyFrameInfo.getForwardControlPointX() + " fY:" + controlPointPair.forwardControlPoint.y
                    + "bP:" + keyFrameInfo.getBackwardControlPointX() + " bY:" + controlPointPair.backwardControlPoint.y);
        }
    }
        ...
        switch (requestCode) {
            case REQUESTCAPTIONSTYLE:
                ...
                if (!isSelectCurCaption) {
                    ...
                    seekMultiThumbnailSequenceView(BackupData.instance().getCurrentTime());
    ...
    private boolean removeAllKeyFrame() {
        if (mCurCaption != null) {
            ...
            boolean removeCaptionOpacity = mCurCaption.removeAllKeyframe(TRACK_OPACITY);
            ...
            return removeStickerTransXSuccess && removeStickerTransYSuccess && removeStickerScaleX
                    && removeStickerScaleY && removeStickerRotZ && removeCaptionOpacity;
        }
        return false;
    }
    ...
    private void removeKeyFrameByCurrentTime(long currentTime) {
        if (mCurCaption != null) {
            ...
            mCurCaption.removeKeyframeAtTime(TRACK_OPACITY, currentTime);
        }
    }
    ...
    private void seekMultiThumbnailSequenceView() {
        long curPos = mStreamingContext.getTimelineCurrentPosition(mTimeline);
        seekMultiThumbnailSequenceView(curPos);
    }

    private void seekMultiThumbnailSequenceView(long time) {
        if (mMultiSequenceView != null) {
            long curPos = time;
            long duration = mTimeline.getDuration();
