...
import com.meishe.sdkdemo.view.FacePointView;
 ...
 * @Description :推镜拍摄页。Super zoom capture page.
...
public class SuperZoomActivity extends BasePermissionActivity implements SurfaceTexture.OnFrameAvailableListener, GLSurfaceView.Renderer {
    ...
    private float[] textureCoords = {
            ...
            1.0f, 0.0f, 0.0f, 1.0f,
            1.0f, 1.0f, 0.0f, 1.0f};
    private final boolean isShowFaceFrame = false;
    ...
    protected int mDetectOrientation = 180;
    ...
    private NvsHumanDetectionHandle mDetectionHandle;
...
//            "rotate",
    ...
    private FacePointView mFacePointView;
    private List<HumanInfo> mHumanInfos = new ArrayList<>();
            ...
            if (activity != null) {
                switch (msg.what) {
                    ...
                    case ASSET_LIST_REQUEST_SUCCESS:
                        ...
                        /*
                         * 遍历修改list中的数据的状态
                         * Iterate through the state of the data in the modified list
                         * */
    ...
    protected void initViews() {
        ...
        String modelPath = "/facemode/ms/ms_face_v1.2.2.model";
        ...
        mDetectionHandle = mEffectSdkContext.createHumanDetectionHandle(destModelDir, licensePath
                , NvsEffectSdkContext.HUMAN_DETECTION_FEATURE_FACE_LANDMARK);
        if (null == mDetectionHandle) {
            finish();
        }
        initUI();
    ...
    private void initUI() {
        ...
        mFacePointView = findViewById(R.id.zoom_face_point);
        /*
         * 设置使用OPENGL ES2.0
         * Set to use OPENGL ES2.0
         * */
        ...
        /*
         * 解决下载刷新和点击冲突问题
         * Resolving download refresh and click conflicts
         * */
        ...
        mCenterHorizontalViewAdapter.setItemClickListener(new CenterHorizontalViewAdapter.ItemClickCallBack() {
            ...
            public void itemclicked(CenterHorizontalViewAdapter.FxViewHolder holder, int position) {
                /*
                 * 点击事件，如果没有下载，则下载
                 * Click event, if not downloaded, download
                 * */
                ...
                if (needDownload) {
                    ...
                    /*
                     * 不需要下载则直接跳转
                     * Jump directly without downloading
                     * */
        ...
        mOrientationEventListener = new OrientationEventListener(mContext, SensorManager.SENSOR_DELAY_NORMAL) {
            ...
            public void onOrientationChanged(int angle) {
                if (mCameraProxy != null) {
                    mDetectOrientation = mCameraProxy.convertDetectOrientation(angle);
                }
            }
        };
    ...
    /**
     * 判断是否有前置摄像头
     * Determine if there is a front camera
     */
    ...
    protected void initListener() {
        mAssetManager.setManagerlistener(new NvAssetManager.NvAssetManagerListener() {
            ...
            public void onDonwloadAssetSuccess(String uuid) {
                /*
                 * 下载完后重新更新
                 * Update again after downloading
                 * */
                ArrayList<NvAsset> localData = getLocalData(NvAsset.ASSET_SUPER_ZOOM);
                for (NvAsset nvAsset : localData) {
                    for (int index = 0, length = fxNames.length; index < length; index++) {
    ...
    private void searchAssetData() {
         ...
         *
         *
         * Search for local data. After retrieving the data, determine if it exists locally, and refresh the adapter after the end.
         *  After clicking download, directly update the adapter, and then store the data (wait for the next time the app starts, perform a local search)
         * */
         ...
         * Search local storage files Update the download status in the list
         * */
       ...
       /* String bundlePath = "meicam";
        ...
        /*
         * 搜索预装，搜索本地（已经下载） 只搜索一次
         * Search pre-installed, search local (already downloaded) search only once
         * */
        ...
        mAssetManager.downloadRemoteAssetsInfo(NvAsset.ASSET_SUPER_ZOOM, -1, NvAsset.AspectRatio_All, 0, 1, 20);
    }
    ...
    /**
     * 无权限界面不可用
     * Unauthorized interface is not available
     */
    ...
    /**
     * 设置切换摄像头是否可用
     * Set whether to switch the camera
     */
    ...
    private boolean startCapturePreview() {
        ...
        if (setupCamera()) {
             ...
             * Has permission (based on whether to open the camera and determine whether there is a camera permission)
             * */
            setCaptureEnabled(true);
...
//        }
    ...
    private void createAndBindTexture() {
         ...
         * Create the Preview Texture needed by the camera
         * */
                    ...
                    if (PackageManager.PERMISSION_GRANTED == ContextCompat.checkSelfPermission(this, android.Manifest.permission.RECORD_AUDIO)) {
                        if (PackageManager.PERMISSION_GRANTED == ContextCompat.checkSelfPermission(this, android.Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                            ...
                            ThreadUtils.getMainHandler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    // 此时activity并没有处于onResume状态，开启预览也没用，延迟一会儿
                                    startCapturePreview();
                                }
                            },500);
                        } else {
...
//        });
    ...
    public void drawFrameToGLView() {
        ...
        if (isInPreview && mNvSuperZoom != null) {
            ...
            mDisplayTex = mNvSuperZoom.preZoomProcess(mCameraPreviewTextures[0], texWidth, texHeight,
                    mCameraProxy.getOrientation(), mFlipHorizontal);
            ...
            GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0,
                    GLES20.GL_TEXTURE_2D, mDisplayTex, 0);
            byte[] byteArray = new byte[texWidth * texHeight * 4];
            ByteBuffer byteBuffer = ByteBuffer.wrap(byteArray);
            byteBuffer.position(0);
            GLES20.glReadPixels(0, 0, texWidth, texHeight, GL10.GL_RGBA, GL10.GL_UNSIGNED_BYTE, byteBuffer);

            detectFace(byteArray, texWidth, texHeight);
     ...
     * Enter the results into GLSurfaceView, which must be run in the opengl environment
     */
    ...
    private Camera.PreviewCallback mPreviewCallback = new Camera.PreviewCallback() {
        ...
        public void onPreviewFrame(final byte[] data, Camera camera) {
            ...
            detectFace(data, bufferWidth, bufferHeight);*/
        }
    };
    ...
    private void detectFace(byte[] data, int width, int height) {
        if (!mDetectFace) {
            if (isShowFaceFrame) {
                mHumanInfos.clear();
                mFacePointView.setHumanInfoList(mHumanInfos, width, height);
            }
            return;
        }
        NvsHumanDetectionHandle.DetectionConfig detectionConfig = new NvsHumanDetectionHandle.DetectionConfig();
        detectionConfig.faceFeature = NvsHumanDetectionHandle.HUMAN_DETECTION_FACE_FEATURE_2D;
        NvsHumanDetectionHandle.HumanFeature feature = mDetectionHandle.detect(data,
                NvsHumanDetectionHandle.HUMAN_DETECTION_VIDEO_FRAME_PIXEL_FORMAT_RGBA8,
                width,
                height,
                width * 4,
                mDetectOrientation,
                detectionConfig);
        if (null != feature) {
            List<NvsHumanDetectionHandle.FaceFeature> faceFeatures = feature.getFaceFeatureArray();
            mHumanInfos.clear();
            ...
            for (NvsHumanDetectionHandle.FaceFeature faceFeature : faceFeatures) {
                if (null == faceFeature) {
                    continue;
                }
                NvsHumanDetectionHandle.FaceData2D faceData2D = faceFeature.FaceData2D();
                if (null == faceData2D) {
                    continue;
                }
                RectF rectF = faceData2D.getBbox();
                RectF faceRect = new RectF();
                faceRect.left = (int) rectF.left;
                faceRect.top = height - (int) rectF.top;
                faceRect.right = (int) rectF.right;
                faceRect.bottom = height - (int) rectF.bottom;

                float centerX = (faceRect.right + faceRect.left) / 2;
                float centerY = (faceRect.bottom + faceRect.top) / 2;
                mAnchorX = centerX / width - 0.5f;
                mAnchorY = -(centerY / height- 0.5f);

                if (isShowFaceFrame) {
                    List<NvsPosition2D> position2Ds = faceData2D.getPoints();
                    List<PointF> pointList = new ArrayList<>();
                    for (NvsPosition2D position2D : position2Ds) {
                        PointF pointF = new PointF();
                        pointF.x = position2D.x;
                        pointF.y = height - position2D.y;
                        pointList.add(pointF);
                    }

                    HumanInfo humanInfo = new HumanInfo(faceRect, pointList, faceData2D.getPitch(),
                            faceData2D.getYaw(), faceData2D.getRoll());
                    mHumanInfos.add(humanInfo);
                    mFacePointView.setHumanInfoList(mHumanInfos, width, height);
                }
            }
        }
    ...
    protected void onDestroy() {
        mAccelerometer = null;
        if (null != mDetectionHandle) {
            mNvsEffectSdkContext.destroyHumanDetectionHandle(mDetectionHandle);
            mDetectionHandle = null;
        }
        /**
         * 释放egl资源
