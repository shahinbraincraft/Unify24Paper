...
public class SelectMusicActivity extends BaseActivity {
    ...
    protected void initData() {
        ...
        Observable.create((ObservableOnSubscribe<List<MusicInfo>>) emitter -> {
            List<MusicInfo> musicData = MusicUtils.getMusicData(Constants.MEDIA_TYPE_AUDIO, MSApplication.getAppContext());
            emitter.onNext(musicData);
        }).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Consumer<List<MusicInfo>>() {
            @Override
            public void accept(List<MusicInfo> musicInfos) throws Exception {
                mLocalMusicFragment.loadAudioData(musicInfos);
            }
        });
...
//        });
    ...
    protected void initListener() {
        ...
        AudioPlayer.getInstance(this).setPlayListener(new AudioPlayer.OnPlayListener( ) {
            ...
            public void onGetCurrentPos(int curPos) {
                ...
                mSelectMusicTime.setText(String.format("%s/%s", TimeFormatUtil.formatUsToString2(curPos), TimeFormatUtil.formatUsToString2(mPlayMusic.getTrimOut( ))));
            }
        });
    ...
    public void playMusic(MusicInfo musicInfo, boolean is_local) {
        ...
        if (mPlayMusic.getDuration( )==0){
            ToastUtil.showToast(mContext,"获取到的音频长度异常");
            return;
        }
        ...
        if (mPlayMusic.isPlay( )) {
            /*
             * 已经选择音乐，处理UI
             * Already selected music, handle UI
             * */
