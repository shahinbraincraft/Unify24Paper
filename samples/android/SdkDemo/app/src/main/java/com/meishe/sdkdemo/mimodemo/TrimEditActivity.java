...
public class TrimEditActivity extends BaseEditActivity {
            ...
            switch (msg.what) {
                case MESSAGE_SCROLLING_FINISH:
                    ...
                    if (mLastX == scaleX) {//停止滑动
                        ...
                        mMimoVideoFragment.playVideo(mCurrentTimeStamp, trimOut);
    ...
    protected void initData() {
        if (NvMiMoContext.getInstance() == null) return;
    ...
    protected void initListener() {
        ...
        mTimelineEditor.setOnScrollListener(new NvsTimelineEditor.OnScrollChangeListener() {
            ...
            public void onScrollX(long timeStamp) {
                mMimoVideoFragment.setStartTime(0);
        ...
        mMimoVideoFragment.setFragmentLoadFinisedListener(new MimoVideoFragment.OnFragmentLoadFinishedListener() {
            @Override
            public void onLoadFinished() {
                mWaitLayout.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        //播放片段，通过片段时间控制
                        if (null != mMimoVideoFragment) {
                            mMimoVideoFragment.seekTimeline(0, 0);
...
//                AppManager.getInstance().finishActivity();
    ...
    private void convertVideo() {
        ...
        if (srcFilePath.startsWith("content:")) {
            srcFilePath = FileUtils.contentPath2AbsPath(getContentResolver(), mSelectShotVideoInfo.getVideoClipPath());
        }
    ...
    private void clearOldSelectData() {
        List<ShotInfo> shotInfos = NvMiMoContext.getInstance().getSelectedMimoData().getShotInfos();
    ...
    protected NvsTimeline initTimeLine() {
        ...
        return MimoTimelineUtil.createTimeline(videoFilePath);
    }
