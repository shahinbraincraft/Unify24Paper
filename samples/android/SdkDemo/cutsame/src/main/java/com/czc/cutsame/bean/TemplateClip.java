...
public class TemplateClip implements Parcelable, Comparable<TemplateClip> {
    ...
    public void update(TemplateClip clip) {
        ...
        setInPoint(clip.getOutPoint());
        setInPoint(clip.getTrimIn());
        setInPoint(clip.getTrimOut());
    ...
    public int compareTo(TemplateClip o) {
        int i = (int) (this.getInPoint() - o.getInPoint());//先按照Inpoint排序
        if (i == 0) {
            return this.getTrackIndex() - o.getTrackIndex();//如果Inpoint相同，按照trackIndex排序
        }
        return i;
    }
}
