 ...
 * @Author : malijia
 * @CreateDate : 2020/6/9.
 * @Description :视频编辑-编辑-FilterKeyFrameView
 * @Description :VideoEdit-FilterKeyFrameView
...
public class FilterKeyFrameView extends LinearLayout implements View.OnClickListener {
     ...
     * view state 1.seeking 2.playing
     */
    ...
    public void setCurrentFx(NvsFx currentFx) {
        ...
        // 关键帧页面打开 重新触发绘制点的操作
        // The keyframe page opens to re-trigger the operation of drawing points
    ...
    /**
     * 滑动过程中 更新所有关键帧view的状态
     * Update the state of all key frame views during the sliding process
     * @param currentTimeStamp
     * @param circleToStampMap
     */
    private void updateAllKeyFrameViewStatusWhenScroll(long currentTimeStamp, HashMap<Point, Long> circleToStampMap) {
        ...
        if (hasKeyframeList) {
            ...
            //刷新当前选中的关键帧图标
            //Refresh the currently selected key frame icon
            //滚动到关键帧位置，关键帧圆点修改
            //Scroll to the key frame position, modify the key frame dot
    ...
    // 滚动关键帧缩略图动画
    // Scroll key frame thumbnail animation
