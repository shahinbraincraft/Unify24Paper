...
public class MimoVideoFragment extends Fragment{
    ...
    private void setLiveWindowRatio(int ratio) {
        ...
        int palyBarHeight = ScreenUtils.dip2px(MSApplication.getContext(),48); //palybar 保证不遮挡画面
        int bottomHeight = ScreenUtils.dip2px(MSApplication.getContext(),145);
        int statusHeight = ScreenUtils.getStatusBarHeight(MSApplication.getContext());//状态栏高度
        int screenWidth = ScreenUtils.getScreenWidth(MSApplication.getContext());//屏宽
        int screenHeight = ScreenUtils.getScreenHeight(MSApplication.getContext());//屏高
