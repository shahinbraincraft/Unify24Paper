 ...
 * @Description :画中画Activity
 * @Description :PictureInPictureActivity
...
//        });
    ...
    public void onClick(View v) {
        ...
        switch (id) {
            ...
            case R.id.zoomInImage:
                /*
                 * 放大视频，放大因子是当前视频的1.25倍
                 * Enlarge the video. The enlargement factor is 1.25 times the current video.
                 * */
                double inScaleFactor = 1.25;
                zoomInAndZoomOut(inScaleFactor);
                break;
            case R.id.zoomOutImage:
                /*
                 * 缩小视频，缩小因子是当前视频的0.8倍
                 * Shrink the video, the reduction factor is 0.8 times the current video
                 * */
                double outScaleFactor = 0.8;
    ...
    private void updateTrackClip() {
        ...
        /*
         * 移除轨道上多余的片段
         * Remove excess clips from the track
         * */
    ...
    /*
     * 计算把某一轨道剩余视频片段补齐
     * Compute the remaining video clips of a track
     * */
    private void addRemainVideoClip() {
        ...
        if (firDuration == 0 || secDuration == 0) {
            /*
             * 有一个轨道片段添加不成功
             * One track clip was not added successfully
             * */
            return;
        }
        ...
        if (result == 0) {
            /*
             * 两个片段时长相同
             * Both clips are the same length
             * */
            return;
        }
        ...
        /*
         * 需要添加的片段数量
         * Number of clips to add
         * */
        long needAddClipCount = result / minValue;
        /*
         * 需要添加的片段时长的一部分长度值
         * The length of the fragment to be added
         * */
        long remainedClipDuration = result % minValue;
    ...
    private void readPicInPicJsonFileInfo(String jsonFilePath, boolean isBundle) {
        try {
            ...
            for (int index = 0; index < count; ++index) {
                ...
                if (isZh(this)) {
                    inPicTemplateAsset.setTemplateName(jsonFileInfo.getName_Zh());
                } else {
    ...
    private void setLiveWindowRatio(int ratio) {
        ...
        switch (ratio) {
            ...
            case NvAsset.AspectRatio_21v9: //21:9
                liveWindowParams.width = screenWidth;
                liveWindowParams.height = (int) (screenWidth * 9.0 / 21);
                break;
            case NvAsset.AspectRatio_9v21: //9:21
                liveWindowParams.width = (int) (newHeight * 9.0 / 21);
                liveWindowParams.height = newHeight;
                break;
            case NvAsset.AspectRatio_18v9: //18:9
                liveWindowParams.width = screenWidth;
                liveWindowParams.height = (int) (newHeight * 9.0 / 18);
                break;
            case NvAsset.AspectRatio_9v18: //9:18
                liveWindowParams.width = (int) (newHeight * 9.0 / 18);
                liveWindowParams.height = newHeight;
                break;
            default: // 16:9
                liveWindowParams.width = screenWidth;
    ...
    private void selectEditDrawRect(RectF shapeRectF) {
        ...
        if (videoType == NvsAVFileInfo.AV_FILE_TYPE_IMAGE) {
            /*
             * 片段是图片，不作裁剪
             * Clips are pictures, no cropping
             * */
