...
public class AgendaItemViewHolder extends RecyclerView.ViewHolder {
    ...
    public void render(MediaData mediaData, final int se, final int position, final OnItemClick onItemClick,boolean isGone) {
        ...
        * */
        ...
        int width = ScreenUtils.getWindowWidth(MSApplication.getContext()) - marginSizeLeftAndRight * 2;
        int marginSizeStart = (int) MSApplication.getContext().getResources().getDimension(R.dimen.select_item_start_end);
        int marginSizeMiddle = (int) MSApplication.getContext().getResources().getDimension(R.dimen.select_item_between);
        ...
        int marginSizeTopAndEnd = ScreenUtils.dip2px(MSApplication.getContext(), 4) / 2;
    ...
    private void setImageByFile(String imagePath, int width) {
        String filePath = FileUtils.getAbsPathByContentUri(MSApplication.getContext(),
                MSApplication.CONTENT_FLAG, imagePath);
