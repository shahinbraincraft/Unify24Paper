...
public class ZoomView extends RelativeLayout {
    // 属性变量 attribute variable
    private float translationX; // 移动X Mobile X
    private float translationY; // 移动Y Mobile Y
    private float scale = 1; // 伸缩比例 Scaling factors
    private float rotation; // 旋转角度 rotation angle
    private float currentRotation;

    private float maxTranslationX;// 移动X 最大值
    private float maxTranslationY;//移动Y最大值
    ...
    //绘制的蒙版框效果路径（黄色的框） Draw the mask frame effect path (yellow box)
    private Path mMaskPath;
    //调节羽化值对应的路径区域 Adjust the path area corresponding to the feather value
    private Path mFeatherPath;
    private Path mWidthPath;
    private Path mHeightPath;
    private Path mCornerPath;
    //顶部预览绘制的线条对应的中心点坐标 Preview the center point coordinates of the drawn line at the top
    private PointF mCenterForMaskView;
    private int mType;
    private float featherWidth;
    //圆角的比例，对最短边的一半的比例(0 - 1) Ratio of rounded corners to half of the shortest side (0-1)
    ...
    //操作羽化值 Manipulate the feather value
    ...
    //调整圆角 Adjust the rounded corners
    private boolean doMaskRoundCorner;

    //保存的是当前选择的蒙版对应的宽高 Saves the width and height of the currently selected mask
    private int currentMaskWidth;
    private int currentMaskHeight;

    private static final String TAG = "ZoomView";
    private int mCenterCircleRadius = 10;
    private boolean touchClick;//用来判断点击事件
    private long touchClickDownTime;
    //属性特技
//    private MeicamVideoFx meicamVideoFx = null;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (i == MotionEvent.ACTION_DOWN) {//蒙版view中心位置，需要根据translate 值重新计算 The center position of the mask view needs to be recalculated according to the translate value
            ...
            mHeightPath = NvMaskHelper.buildMaskHeightPath(centerDownCenter, endRotation, maskInfoData.getMaskWidth(), maskInfoData.getMaskHeight(), ScreenUtils.getScreenWidth(), mFeatherIconDis);
            ...
            if (moveType == 1) {
                if (doFeather) {
                    ...
                    //羽化值变化和高度变化相反 The change of eclosion value is opposite to the change of height
...
//                    translationY = translationY + translationDValue.y;
                    ...
                    //这里要计算在当前旋转角度下，对应方向上的距离 So we're going to calculate the distance in that direction at the current rotation Angle
                    ...
                    //这里要计算在当前旋转角度下，对应方向 And what we're going to do here is we're going to calculate the direction of the current rotation
                    ...
                    if (curHeight <= 0) {
                        curHeight = 0;
                    } else if (curHeight >= ScreenUtils.getScreenWidth() - mMaskWidthIconDis * 4) {
                        curHeight = ScreenUtils.getScreenWidth() - mMaskWidthIconDis * 4;
                    }
...
//            Log.e(TAG, "ACTION_MOVE rotation = " + rotation + "   scale = " + scale);
    ...
    private Path getMaskPath(MaskInfoData maskInfoData, PointF centerDownCenter) {
        ...
        if (mType == MaskRegionInfoData.MaskType.LINE) {
            //线性蒙版拖拽位置的特殊性，需要计算横线的上下一段距离触发拖拽操作 The drag position of the linear mask is special, so it is necessary to calculate the upper and lower distance of the horizontal line to trigger the drag operation
    ...
    // 触碰两点间距离 Distance between two points of contact
    ...
    private int buildChangeValueForWidth(float mRotation, int moveX, int moveY) {
        int moveValue = 0;
        //计算角度为-360 - 360  顺时针 逆时针 两项 Calculate the Angle of -360 -360 clockwise and anticlockwise
