 ...
 * @Description :视频编辑-音乐-Activity
 * @Description :VideoEdit-Music-Activity
...
public class MusicActivity extends BaseActivity {
    ...
    protected void initData() {
        ...
        if (musicInfos != null) {
            for (int i = 0; i < musicInfos.size(); ++i) {
                 ...
                 * If there is music in the first frame, then you have to show the option to set the music (because you just entered the page and stopped at the first frame)
                 * */
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == Constants.ACTIVITY_START_CODE_MUSIC_SINGLE) {
                /*
                 * 处理单段音乐
                 * Handling a single piece of music
                 * */
                ...
                /*
                 * 处理多段音乐
                 * Handling multiple pieces of music
                 * */
                MusicInfo musicInfo = (MusicInfo) data.getSerializableExtra("select_music");
    ...
    private void updatePlaytimeText(long playTime) {
        long totalDuration = m_timeLine.getDuration();
        String totalStr = TimeFormatUtil.formatUsToString1(totalDuration);
    ...
    /**
     * 获取指定位置的音频片段
     * Get the audio clip at the specified location
     *
     * @param curPos
     * @return
     */
    ...
    /**
     * 根据一个音频入点时间获取下一个音频的入点
     * Get the in point of the next audio according to the time of one audio in point
     *
     * @param curInPoint
     * @return
     */
    ...
    /**
     * 根据一个音频入点时间获取前一个音频的出点
     * Get the out point of the next audio according to the time of one audio in point
     *
     * @param curInPoint
     * @return
     */
    ...
    private void addOneMusic(MusicInfo musicInfo, boolean single) {
        ...
        if (single) {
             ...
             * Process data (if overlap, process trimOut of current music)
             * */
            for (int i = 0; i < m_musicTrack.getClipCount(); ++i) {
                NvsAudioClip audioClip = m_musicTrack.getClipByIndex(i);
                if (audioClip == null) {
        ...
        /*
         * 把已经应用的主题中的音乐去掉
         * Remove music from the theme you have applied
         * */
    ...
    private void changeInPoint(long inPointChanged) {
        ...
        if (musicClip != null && cur_musicInfo != null) {
             ...
             * Processing data (if overlapping, processing the inPoint of the current music)
             * */
    ...
    private void addAudioClip() {
        ...
        /*
         * 防止有添加不成功的
         * Prevent unsuccessful additions
         * */
        musicInfosClone.clear();
    ...
    /**
     * 设置是否淡入淡出
     * Set whether to fade in and out
     *
     * @param have_fade
     */
