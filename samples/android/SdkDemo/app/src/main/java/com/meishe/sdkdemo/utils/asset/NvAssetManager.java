...
public class NvAssetManager implements NvHttpRequest.NvHttpRequestListener, NvsAssetPackageManager.AssetPackageManagerCallback {
    ...
    /**
     * 是否同步安装素材，默认同步安装。
     * Whether to install the materials synchronously. The default installation is synchronous.
     */
        ...
        if (assetType == NvAsset.ASSET_FACE1_STICKER) {
            ...
            if (isSyncInstallAsset) {
                ...
                if (BuildConfig.DEBUG) {
                    Log.e(TAG, " |||errorcode ==" + error + " type:" + asset.getPackageType() + "== filePath:" + filePath);
                }
                if (error == NvsAssetPackageManager.ASSET_PACKAGE_MANAGER_ERROR_NO_ERROR) {
                    ...
                    if (version > asset.version) {
                        ...
                        if (error == NvsAssetPackageManager.ASSET_PACKAGE_MANAGER_ERROR_NO_ERROR || error == NvsAssetPackageManager.ASSET_PACKAGE_MANAGER_ERROR_WORKING_INPROGRESS) {
                ...
                if (manager.getAssetPackageStatus(asset.uuid, asset.getPackageType()) == NvsAssetPackageManager.ASSET_PACKAGE_MANAGER_ERROR_ALREADY_INSTALLED) {
                    ...
                    if (assetInfo == null) {
                         ...
                         * This will not be called under normal circumstances,
                         * because the installed material can definitely be found.
                         * */
                    } else {
    ...
    private void addRemoteAssetData(List<BaseDataBean> resultsArray, int assetType) {
        ...
        for (BaseDataBean assetInfo : resultsArray) {
            ...
            String packageUrl = assetInfo.getPackageUrl();
