...
import static com.czc.cutsame.util.CustomConstants.REQUEST_CODE_1;
import static com.czc.cutsame.util.CustomConstants.SELECTED_TYPE;
import static com.czc.cutsame.util.CustomConstants.TEMPLATE_CLIP;
import static com.czc.cutsame.util.CustomConstants.TYPE_ONE_FINISH;
...
import com.czc.cutsame.TailorFreeActivity;
...
import com.czc.cutsame.pop.CutCaptionEditPop;
...
public class CutEditorVpFragment extends BaseMvpFragment<CutEditorVpPresenter> implements CutEditorVpView {
    ...
    public void changClipTrimInfo(List<TemplateClip> clipList,long newTrimIn,long newTrimOut) {
        ...
        List<NvsAssetPackageManager.NvsTemplateFootageCorrespondingClipInfo> templateVideoClip = mPresenter.getTemplateVideoClip(clipList);
    ...
    public void getCaptionData(List<TemplateCaptionDesc> captionClipList) {
        ...
        mCutCaptionAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, final int position) {
                ...
                final TemplateCaptionDesc captionDesc = mCaptionData.get(position);
                if (mCutCaptionAdapter.getSelectIndex() == mCutCaptionAdapter.getBeforeSelectIndex()) {
                    showCaptionEditPop(captionDesc, position);
                } else {
        ...
        mPresenter.getCaptionBitmap(mTemplateId,mTemplateType);
    }

    /**
     * 外部更新字幕数据
     * @param captionClipList 字幕集合
     */
     ...
     */
    private void showCaptionEditPop(final TemplateCaptionDesc captionDesc, final int position) {
        ...
        mCaptionEditPop.setEventListener(new CutCaptionEditPop.EventListener() {
            ...
            public void onConfirm(String text) {
                mPresenter.changeCaptionText(captionDesc, text);
     ...
     */
    @Override
    public void getCaptionBitmap(long inPoint, Bitmap bitmap) {
        if (CommonUtils.isEmpty(mCaptionData)) {
            return;
        }
        for (int index = 0; index < mCaptionData.size(); index++) {
            TemplateCaptionDesc captionDatum = mCaptionData.get(index);
            if (captionDatum.getInPoint() == inPoint) {
                captionDatum.setBitmap(bitmap);
     ...
     */
    @Override
    public void getVideoData(List<NvsAssetPackageManager.NvsTemplateFootageCorrespondingClipInfo> templateClipList) {
        ...
        mCutVideoAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, final View view, final int position) {
                ...
                final NvsVideoClip videoClip = EditorController.getInstance().getVideoClipByIndex(selectedClipInfo.trackIndex, selectedClipInfo.clipIndex);
                if (videoClip == null) {
                ...
                if (!selectedClipInfo.canReplace) {
                    ...
                    EditorController.getInstance().seekTimeline(videoClip.getInPoint());
                    if (mSeekPositionListener != null) {
                        mSeekPositionListener.seekPosition(videoClip.getInPoint());
                    }
                    return;
                }
                EditorController.getInstance().seekTimeline(videoClip.getInPoint());
        ...
        mVideoEditPop.setEventListener(new VideoEditPop.EventListener() {
            ...
            public void onReplace() {
                if (getActivity() instanceof CutSameEditorActivity) {
                    ...
                    TemplateClip templateClip = ((CutSameEditorActivity) getActivity()).getTemplateClip(selectedClipInfo.trackIndex, selectedClipInfo.inpoint);
