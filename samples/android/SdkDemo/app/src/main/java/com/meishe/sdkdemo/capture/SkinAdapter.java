...
public class SkinAdapter extends RecyclerView.Adapter<SkinAdapter.ViewHolder> {
     ...
     * 只是存放磨皮的数据
     */
    private List<ArBean> mTmpBeautyData;
    private Switch mSwitchView;

    public SkinAdapter(Context context, List<ArBean> skinDataList, List<ArBean> dataList) {
        mContext = context;
        mDataList = dataList;
    ...
    public void setDataList(List<ArBean> data) {
    ...
    public ArBean getItem(String tag) {
        ...
        for (int i = 0; i < mDataList.size(); i++) {
            ArBean beautyShapeDataItem = mDataList.get(i);
        ...
        if (mDataList != null) {
            for (int i = 0; i < mDataList.size(); i++) {
                ...
                if (!TextUtils.isEmpty(name) && name.startsWith(mContext.getResources().getString(R.string.whitening))) {
                    beautyShapeDataItem.setName(newName);
                    notifyItemChanged(i);
                    break;
                }
            }
    ...
    public int getItemViewType(int position) {
        if (position == 0 && null != shapeDataKindItem) {
            ...
            ArBean beautyShapeDataItem = mDataList.get(position);
            if (beautyShapeDataItem != null && beautyShapeDataItem instanceof PointBean) {
                return TYPE_POINT;
            }
...
//                holder.shape_icon_layout.setBackgroundResource(R.drawable.shape_beauty_menu_select);
            ...
            final ArBean item = mDataList.get(position);
            ...
            if (position == 0) {
                if (isHandClick) {
                    ...
                    if (!isExtendState) {
                        ...
                        for (int i = 0; i < mTmpBeautyData.size(); i++) {
                            ArBean beautyShapeDataItem = mTmpBeautyData.get(i);
                            if (beautyShapeDataItem == null) {
                                continue;
                            }
                ...
                holder.shape_icon.setImageResource(item.getResId());
                holder.shape_name.setText(item.getName());
                if (mIsEnable) {
    ...
    private void hideStrength() {
        for (int i = 0; i < mDataList.size(); i++) {
            ArBean beautyShapeDataItem = mDataList.get(1);
            if (beautyShapeDataItem.getName().equals(mContext.getResources().getString(R.string.strength_1))
                    || beautyShapeDataItem.getName().equals(mContext.getResources().getString(R.string.advanced_strength_1))
                    || beautyShapeDataItem.getName().equals(mContext.getResources().getString(R.string.advanced_strength_2))
                    || beautyShapeDataItem.getName().equals(mContext.getResources().getString(R.string.advanced_strength_3))
                    || beautyShapeDataItem.getName().equals(mContext.getResources().getString(R.string.blackPoint))) {
                ArBean remove = mDataList.remove(1);
                i--;
            }
        }
        mSelectedPos = 0;
    ...
    public ArBean getSelectedBeautyTempData() {
