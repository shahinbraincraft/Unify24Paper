...
import com.meishe.sdkdemo.mimodemo.common.dataInfo.MusicInfo;
import com.meishe.sdkdemo.mimodemo.common.dataInfo.RecordAudioInfo;
import com.meishe.sdkdemo.mimodemo.common.dataInfo.StickerInfo;
...
import com.meishe.sdkdemo.mimodemo.common.template.model.FxClipInfo;
...
public class MimoTimelineUtil {
    private static String TAG = "MimoTimelineUtil";
    ...
    public static NvsTimeline createTimeline(String clipPath){
        NvsTimeline timeline = newTimeline(MimoCommonUtil.getVideoEditResolution(Constants.AspectRatio.AspectRatio_16v9));
        if(timeline == null) {
            Log.e(TAG, "failed to create timeline");
            return null;
        }
    ...
    private static void addShotVideo(NvsTimeline timeline,MiMoLocalData selectTemplate){
        ...
        for (int index = 0;index < trackClipCount;index++){
            ...
            //添加主轨道片段
            long inPoint = 0;
            long timelineDuration = timeline.getDuration();
            ...
            if (!TextUtils.isEmpty(mainTrackVideoInfo.getFilter()) && clipCount > 1) {
                appendTimelineFilter(timeline, timelineDuration, mainTrackVideoInfo.getFilter());
            }
    ...
    private static void appendTimelineFilter(NvsTimeline timeline, long inPoint, String filter) {
        if (timeline == null || TextUtils.isEmpty(filter)) {
            return;
        }
        long duration = timeline.getDuration()- inPoint;
        Log.d(TAG, " shot =appendTimelineFilter start:" + inPoint + " end:" + duration + " filter:" + filter);
        if (isBuiltInFx(filter)) {
            timeline.addBuiltinTimelineVideoFx(inPoint, duration, filter);
        } else {
            timeline.addPackagedTimelineVideoFx(inPoint, duration, filter);
        }
    }
    ...
    private static void setTimelineData(NvsTimeline timeline,MiMoLocalData selectTemplate){
        ...
        long titleFilterDuration = MiMoFileUtils.millisecondToMicrosecond(selectTemplate.getTitleFilterDuration());
        addTimelineFilter(timeline,titleFilter,0,titleFilterDuration);
        //添加片头组合字幕
        String titleCaption = selectTemplate.getTitleCaption();
        long titleCaptionDuration = MiMoFileUtils.millisecondToMicrosecond(selectTemplate.getTitleCaptionDuration());
        ...
        long endingFilterLen = MiMoFileUtils.millisecondToMicrosecond(selectTemplate.getEndingFilterLen());
        ...
        //添加中间的组合字幕
        long inPoint = 0;
        for (ShotInfo shotInfo : selectTemplate.getShotInfos()) {
            String compoundCaptionUuid = shotInfo.getCompoundCaption();
            if (!TextUtils.isEmpty(compoundCaptionUuid)) {
                addCompoundCaption(timeline, compoundCaptionUuid, inPoint * 1000, (long) shotInfo.getDuration() * 1000);
            }
            inPoint += shotInfo.getDuration();
        }
    }
    //添加组合字幕
    ...
    private static void appendAudioClip(NvsTimeline timeline,MiMoLocalData selectTemplate){
        ...
        long musicDuration = MiMoFileUtils.millisecondToMicrosecond(selectTemplate.getMusicDuration());
    ...
    private static long appendVideoClip(NvsVideoTrack videoTrack,
                                        ...
                                        long inPoint,boolean timelineFilterFlag){
        if (videoTrack == null){
            return 0;
        }
        if (trackClipInfo == null){
        ...
        Log.d(TAG, "shot = clipTrimIn:" + clipTrimIn + " inPoint:" + inPoint + " clipTrimOut:" + clipTrimOut + ",videoClipPath = " + videoFilePath);
    ...
    private static void appendFilter(NvsVideoClip videoClip,String filterName){
        ...
        if (isBuiltInFx){
            nvsVideoFx = videoClip.appendBuiltinFx(filterName);
        }else {
            nvsVideoFx = videoClip.appendPackagedFx(filterName);
        }
        if (null!=nvsVideoFx){
            nvsVideoFx.setAbsoluteTimeUsed(true);
        }
    }
    //对视频片段作变速
    ...
    public static boolean applyTheme(NvsTimeline timeline, String themeId) {
        ...
        MimoTimelineUtil.buildTimelineMusic(timeline, null);
        return true;
    }
